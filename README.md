# mc-select-player-chat-color

为 Paper 服务器中的指定玩家设置原版聊天颜色。设置后，玩家发送的整条聊天内容，包括 `<玩家名>` 与消息正文，都会使用同一种颜色。

## 使用方法

OP 或拥有 `mc-select-player-chat-color.admin` 权限的管理员可以执行：

```text
/mc-select-player-chat-color <玩家> <颜色|reset>
```

示例：

```text
/mc-select-player-chat-color rainyxin yellow
/mc-select-player-chat-color rainyxin dark_aqua
/mc-select-player-chat-color rainyxin reset
```

在线玩家可直接设置。离线玩家必须已经被当前服务端缓存，以确保颜色绑定到正确的 UUID。`reset` 会恢复该玩家的默认聊天样式。

支持全部 16 种原版命名颜色：

```text
black, dark_blue, dark_green, dark_aqua, dark_red, dark_purple, gold, gray,
dark_gray, blue, green, aqua, red, light_purple, yellow, white
```

## 工作原理

```mermaid
classDiagram
    direction TB

    class 聊天颜色插件主类 {
        - 玩家 UUID 到颜色的并发映射
        - 已排序的可用颜色名称列表
        - 纯文本组件序列化器常量
        - 管理员权限节点常量
        + 启用插件()
        + 处理异步聊天事件(事件)
        + 执行管理员命令(发送者, 参数)
        + 提供命令补全(发送者, 参数)
        - 查找在线或已缓存玩家(名称)
        - 加载玩家颜色()
        - 保存玩家颜色()
        - 发送命令用法(发送者)
    }

    class JavaPlugin抽象父类 {
        <<抽象父类>>
        + 保存默认配置()
        + 取得配置()
        + 取得服务器()
    }

    class 事件监听器 {
        <<接口>>
    }

    class 命令执行器 {
        <<接口>>
        + 执行命令(发送者, 命令, 参数)
    }

    class 命令补全器 {
        <<接口>>
        + 提供补全(发送者, 命令, 参数)
    }

    class Paper服务器 {
        在线玩家集合
        已缓存离线玩家
        插件管理器
        + 精确查找在线玩家(名称)
        + 查找已缓存离线玩家(名称)
        + 注册事件(监听器)
        + 取得插件命令(名称)
    }

    class 插件命令 {
        命令名称
        命令用法
        + 设置执行器(命令执行器)
        + 设置补全器(命令补全器)
    }

    class 命令发送者 {
        <<接口>>
        + 是否拥有权限(权限节点)
        + 发送消息(组件)
    }

    class 管理员权限 {
        <<权限节点>>
        mc-select-player-chat-color.admin
        默认仅 OP
    }

    class 颜色命令参数 {
        玩家名称
        颜色名称或 reset
        + 转换为小写()
        + 是否为 reset()
    }

    class 命令补全候选 {
        在线玩家名称
        16 种颜色名称
        reset
        + 按已输入前缀过滤()
        + 不区分大小写排序()
    }

    class 玩家颜色映射 {
        <<并发哈希映射>>
        玩家 UUID
        原版命名颜色
        + 查询(UUID)
        + 写入(UUID, 颜色)
        + 移除(UUID)
    }

    class 配置文件 {
        <<config.yml>>
        玩家 UUID 到颜色名称
        + 读取()
        + 写入临时文件()
        + 原子替换或普通替换()
    }

    class 运行时配置对象 {
        <<FileConfiguration>>
        根路径 player-colors
        + 删除旧映射()
        + 创建配置节()
        + 保存到临时文件()
    }

    class 配置节 {
        路径 player-colors
        UUID 字符串键
        颜色名称值
        + 枚举键()
        + 读取字符串(UUID)
        + 写入(UUID, 颜色名)
    }

    class 临时配置文件 {
        <<config.yml.tmp>>
        + 由 YAML 写入器创建
        + 成功后替换正式配置
        + 失败后删除
    }

    class 文件系统 {
        + 原子移动(临时文件, 正式文件)
        + 普通替换(临时文件, 正式文件)
        + 删除临时文件()
    }

    class 配置写入异常 {
        <<IOException>>
        + 触发临时文件清理()
        + 保留内存中的本次颜色变更()
    }

    class 服务器日志 {
        + 记录严重配置保存错误()
        + 记录临时文件清理警告()
    }

    class UUID {
        <<玩家稳定标识>>
        + 从配置字符串解析()
        + 转换为配置键()
    }

    class 原版命名颜色 {
        <<Adventure 值对象>>
        原版颜色名称索引
        + 按名称查询(颜色名)
        + 取得颜色名称()
    }

    class 颜色名称索引 {
        <<NamedTextColor.NAMES>>
        16 种原版命名颜色
        + 按名称取得颜色()
        + 枚举全部名称()
    }

    class 异步聊天事件 {
        发送者 UUID
        聊天消息组件
        接收者集合
        取消状态
        + 设置聊天渲染器(渲染器)
    }

    class 聊天渲染器 {
        <<Paper 接口>>
        + 创建无接收者差异渲染器(渲染器)
        + 渲染(发送者, 显示名, 消息, 接收者)
    }

    class 无接收者差异渲染函数 {
        <<Lambda>>
        发送者
        显示名
        消息组件
        + 生成同色完整消息()
    }

    class 聊天消息组件 {
        玩家名文本
        消息正文文本
        原版命名颜色样式
        + 创建同色完整消息()
    }

    class Adventure文本组件 {
        纯文本内容
        颜色样式
        + 设置颜色()
        + 追加文本()
    }

    class 纯文本组件序列化器 {
        <<Adventure 工具>>
        + 转换为纯文本(消息)
    }

    class 在线玩家 {
        UUID 唯一标识
        玩家名
        在线状态
    }

    class 已缓存离线玩家 {
        UUID 唯一标识
        玩家名
        历史缓存状态
    }

    class 聊天接收者 {
        <<Audience>>
        + 接收聊天组件()
    }

    class 游戏聊天栏 {
        + 显示默认或彩色聊天组件()
    }

    JavaPlugin抽象父类 <|-- 聊天颜色插件主类
    事件监听器 <|.. 聊天颜色插件主类
    命令执行器 <|.. 聊天颜色插件主类
    命令补全器 <|.. 聊天颜色插件主类

    聊天颜色插件主类 --> Paper服务器 : 取得玩家、命令和插件管理器
    Paper服务器 --> 插件命令 : 提供同名命令
    插件命令 --> 命令执行器 : 调用
    插件命令 --> 命令补全器 : 调用
    命令发送者 --> 管理员权限 : 检查
    聊天颜色插件主类 ..> 命令发送者 : 处理管理员输入
    聊天颜色插件主类 --> 颜色命令参数 : 解析两个参数
    颜色命令参数 --> 原版命名颜色 : 请求颜色校验
    聊天颜色插件主类 --> 命令补全候选 : 生成补全结果
    命令补全候选 --> 在线玩家 : 读取在线名称
    命令补全候选 --> 颜色名称索引 : 读取颜色和 reset

    聊天颜色插件主类 *-- 玩家颜色映射 : 持有
    聊天颜色插件主类 --> 配置文件 : 读取并安全保存
    聊天颜色插件主类 --> 运行时配置对象 : 读写配置内容
    运行时配置对象 --> 配置文件 : 保存目标
    配置文件 *-- 配置节 : 包含 player-colors
    运行时配置对象 --> 配置节 : 创建和清空
    配置文件 --> 临时配置文件 : 先写入
    临时配置文件 --> 文件系统 : 请求替换或清理
    文件系统 --> 配置文件 : 替换正式文件
    文件系统 --> 配置写入异常 : 替换失败时抛出
    配置写入异常 --> 服务器日志 : 记录错误与清理警告
    配置文件 --> 玩家颜色映射 : 持久化 UUID 映射
    配置节 --> UUID : 键的解析和序列化
    玩家颜色映射 --> UUID : 使用为键
    玩家颜色映射 --> 原版命名颜色 : 存储颜色值
    聊天颜色插件主类 --> 原版命名颜色 : 校验颜色名称
    颜色名称索引 --> 原版命名颜色 : 按名称取得颜色值

    聊天颜色插件主类 ..> 在线玩家 : 精确查找在线玩家
    聊天颜色插件主类 ..> 已缓存离线玩家 : 查找已缓存离线玩家
    在线玩家 --> UUID : 持有
    已缓存离线玩家 --> UUID : 持有
    聊天颜色插件主类 --> 异步聊天事件 : 以 HIGHEST 优先级处理
    异步聊天事件 --> 在线玩家 : 提供发送者
    异步聊天事件 --> 聊天渲染器 : UUID 有颜色时设置
    聊天渲染器 *-- 无接收者差异渲染函数 : 包装渲染逻辑
    聊天渲染器 --> 纯文本组件序列化器 : 展平消息文本
    聊天渲染器 --> 原版命名颜色 : 为整条聊天设置颜色
    无接收者差异渲染函数 --> Adventure文本组件 : 创建同色消息
    聊天渲染器 --> 聊天消息组件 : 创建完整同色消息
    聊天消息组件 --> Adventure文本组件 : 表示为 Component
    异步聊天事件 --> 聊天接收者 : 提供接收者集合
    聊天消息组件 --> 游戏聊天栏 : 由 Paper 分发显示
```

颜色映射由 UUID 而非玩家名保存，因此玩家改名后仍会保留已设置的聊天颜色。

## 环境与构建

本项目面向 Paper `26.2.build.87-stable` 与 Java 25，使用 Paper 的 `AsyncChatEvent` 和 `ChatRenderer`。不保证旧版 Paper、Spigot 或 Purpur 的兼容性。

```bash
mvn clean package
```

构建产物为 `target/mc-select-player-chat-color-1.0.0.jar`。将它放入服务器的 `plugins/` 目录后重启服务器即可加载。
